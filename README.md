...
<p>このリポジトリは、findコマンドの-execオプションを使用してgrepコマンドにファイルパスを渡して実行します</p>
</div>
...
<div class="wp-block-group">
<p>上記のコマンドをインストール済みの方は、以下のコマンドを実行してリポジトリからダウンロード後、ディレクトリを移動し、findコマンドの-execオプションを使用してgrepコマンドを実行してみてください</p>
...
<p>上記のシェルは、カレントディレクトリ以下のファイルで、helloが含まれるファイルを{}のファイルパスからできる限り1回のプロセスで実行するという意味です</p>
